...
public class MiniGoogle{
    ...
    private double parent(double i) { 
        return (i-1) / 2; 
    }
    
    private double lchild(double i) {
        return 2 * i + 1; 
    }
    
    private double rchild(double i) {
        return 2 * i + 2;
    }
    
    private boolean isLeaf(double i) { 
        return (lchild(i) >= next); 
    }
    
    private boolean isRoot(double i) {
        return i == 0;
    }
    
    // inserts an article into the array at the next available location
    private void insert(Article a) {
    ...
    public double getMax() {
        ...
        double i = 0;                   // i is location of new key as it moves down tree
        
        double mc = maxChild(i); 
